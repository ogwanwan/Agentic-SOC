...
- 조회 결과 0건(예: "이 IP의 network 기록 없음")을 판단에 쓰려면 증거로 적되 `raw_refs`는 비우고, description에
  도구·조건·구간과 0건을 명시하며 약한 신호(±0.05)로만 반영한다. 판단에 안 쓰는 0건은 notes에.
  - 왜: 예전에는 "0건은 증거가 아니다"와 신뢰도 표의 "단순 통신 없음 확인 ±0.05"가 서로 부딪혔고, LLM은 후자를
    따라 0건을 증거로 적고 있었다. 실제 동작에 맞춰 문장을 통일했다(2026-09-25). 이런 증거는 참조가 없어
    보고서의 `Raw reference validation`이 `incomplete`로 나올 수 있다 — 원본 추적 오류가 아니다.
...
- **B′. 코드와 중복된 기준 숫자 → 코드 상수를 프롬프트에 자동으로 채우기** (다음에 할 것): 원칙 7(실패 5회·계정 2개·
  1~4회·탐침 1~4건), 원칙 9(POST 10회·경로 20개·4xx 과반), 원칙 2의 구간 숫자가 yaml과 코드에 두 번 적혀 있다. 기준값을
  바꿀 때 한쪽만 고치면 LLM 판정과 종료 관문이 엇갈린다.
  - 처음 안(B)은 yaml에서 숫자를 빼고 "`[원칙 N 기준]`을 따르라"로 바꾸는 것이었으나 **채택하지 않는다**: 코드는 IP 하나로
    거른 조회에서만 기준을 계산하므로, 그 외 조회에서는 yaml의 숫자가 LLM의 유일한 판단 근거다.
  - **B′ 설계**: yaml에는 `실패 {P7_FAILURES}회 이상`처럼 자리 표시만 두고, `build_system_prompt()`가 코드 상수
    (`fetch_auth_log.BRUTEFORCE_FAILURES`·`BRUTEFORCE_ACCOUNTS`, `fetch_web_log.AUTH_POST_THRESHOLD`·`SCAN_PATH_THRESHOLD`,
    `prompts.LAYER_WINDOW_PADS`·`AUTH_LOOKBACK_*`)로 채운다. 숫자는 코드 한 곳에만 있고 LLM에게는 계속 보인다.
  - 검증: 조립된 시스템 프롬프트가 적용 전과 **글자 단위로 같은지** 오프라인 테스트로 확인(같으면 LLM 재검증 불필요).
  - 시점: 회의에서 기준값을 바꾸기로 하면 B′을 먼저 적용하고 상수만 바꾼다. 유지하면 통합 후.
- **C. 반복 문장** (필요 시): 거부 대응(원칙 4 ↔ 거부 안내문), 증거 중복·신뢰도 부풀리기 금지(원칙 4·7·rules·거부 안내문),
  Q1~Q3 기록 방식(원칙 7 ↔ rules), 원칙 9 [침해 신호] 두 문단. 판정을 흔든 사례가 없고 일부 반복(거부 안내문의 "부풀리지
  말라")은 가장 필요한 순간에 다시 보여 주는 효과가 있어, 프롬프트를 크게 개편할 때 함께 정리한다.
- 이미 적용: A1(0건 증거 규칙 통일), A2(도구 기준과 같은 판정의 신뢰도 면제를 원칙 4에 명시) — `dce17fb`.
