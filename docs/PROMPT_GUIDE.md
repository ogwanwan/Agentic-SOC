...
- **B. 코드와 중복된 기준 숫자**: 원칙 7(실패 5회·계정 2개·1~4회·탐침 1~4건), 원칙 9(POST 10회·경로 20개·4xx 과반),
  원칙 2의 구간 숫자가 yaml과 코드에 두 번 적혀 있다. 기준값을 바꿀 때 한쪽만 고치면 LLM 판정과 종료 관문이 엇갈린다.
  yaml에서는 "`[원칙 N 기준]` 결과를 따른다"로 바꾸고 숫자는 코드에만 두는 안. 회의에서 기준값을 바꾸기로 하면 함께 적용.
- **C. 반복 문장**: 거부 대응(원칙 4 ↔ 거부 안내문), 증거 중복·신뢰도 부풀리기 금지(원칙 4·7·rules·거부 안내문),
  Q1~Q3 기록 방식(원칙 7 ↔ rules), 원칙 9 [침해 신호] 두 문단. 통합 후 정리.
